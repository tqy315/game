...
def _astar_shortest_path(start_board, max_states=300000):
...
def _greedy_hint(board):
    """Fallback: pick the neighbor with best heuristic (greedy best-first)."""
    best_move = None
    best_h = float('inf')
    for neighbor, move_num in _get_neighbors(board):
        h = _heuristic(neighbor)
        if h < best_h:
            best_h = h
            best_move = move_num
    return best_move
...
def get_hint(board):
    """返回下一步应该移动的数字（最优提示），如果 A* 超时则退化为贪心提示。"""
    path = _astar_shortest_path(board)
    if path and len(path) > 0:
        return path[0]
    if path is not None and len(path) == 0:
        return None  # Already solved
    # A* timed out or no path found, use greedy fallback
    return _greedy_hint(board)
